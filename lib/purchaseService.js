CodeBashApp.purchaseService = (function () {
  ...
  var init = function () {
      return {
          ...
          findPurchaseByDeliveryStatus: function(status)
          {
            return Purchase.find({"status":"saved","deliveryStatus":status}).fetch();
          },
          findPurchase: function()
          {
            return Purchase.find().fetch();
          },
          findPurchasebySellerId: function(id)
          {
            return Purchase.find({"sellerId":id}).fetch();
          },
        ...
        updatePurchase: function(id,purchaseId,paymentStatus,deliveryStatus,totalCost,status)
        {
          if(purchaseId)
          {
            id = Purchase.find({"purchaseId":purchaseId}).fetch()[0]._id;
          }
          if(deliveryStatus)
          {
