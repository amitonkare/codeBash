CodeBashApp.buyerDetailsEditBuyerValidate=function(){
	...
	if(validate == 'false')
	{
		...
		{
			if($.trim($("#buyerAddress").val()).length == 0 )
			{
				flagArray.push(20);
			}
		}
		...
		if(specialCharRegex.test($("#buyerName").val()))
		{
			flagArray.push(25);
		}
	...
	if(validate != 'true')
	{
		for (var i = 0; i < flagArray.length; i++) {
			...
			switch(flagArray[i])
			{
				...
				$("#buyerNameGroup").addClass('form-group has-error has-feedback');                 
				$("#buyerNameSpan").html('name cannot have special symbols');
				break;
			}
		}
	}   
