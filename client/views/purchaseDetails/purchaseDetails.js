...
Template.purchaseDetails.events({
	"submit #addToCart":function(event)
	{
		...
		if(flag == 1)
		{
			...
			tempObj.quantity ='1' ;
			tempObj.cost = '';
			tempObj.individualTotal = Number(tempObj.cost * tempObj.quantity);
			temp.insert(tempObj);
			flag = 1;
		}
	},
...
"click #printPurchase":function()
{
	...
	if(flag == 0)
	{
		...
		sellerName = CodeBashApp.sellerDetailsService.getInstance().findSellerNameById(sellerName)[0].name;
